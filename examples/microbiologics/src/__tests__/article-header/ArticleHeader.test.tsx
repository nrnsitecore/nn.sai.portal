...
describe('ArticleHeader Card variant', () => {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  let ArticleHeaderCard: React.ComponentType<any>;

  beforeAll(async () => {
    const mod = await import('../../components/article-header/ArticleHeader');
    ArticleHeaderCard = mod.Card;
  });

  it('renders title, cta, and summary in card layout', () => {
    render(
      <ArticleHeaderCard
        {...fullProps}
        fields={{
          ...
          summary: {
            value:
              'Our team of experts in drug discovery and testing is dedicated to developing innovative solutions.',
          },
        }}
        externalFields={{
          ...fullProps.externalFields,
          pageHeaderTitle: { value: 'Advanced Antibacterial & Antifungal Discovery' },
        }}
      />,
    );

    expect(
      screen.getByRole('heading', { name: /Advanced Antibacterial & Antifungal Discovery/i }),
    ).toBeInTheDocument();
    expect(screen.getByText(/Advancing Life-Saving Therapies/i)).toBeInTheDocument();
    expect(screen.getByText(/Our team of experts/i)).toBeInTheDocument();
